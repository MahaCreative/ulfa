...
export default function Anggota(props) {
    ...
    const reload = useCallback(debounce((query) => {
        Inertia.get(route('anggota'), query, {
            preserveState: true,
          ...
          <div className='px-4 border border-emerald-300 rounded-lg'>
              <div className='flex justify-between items-center py-2.5'>
                  <div className='flex gap-3'>
                      <button onClick={addModalHandler} className='rounded-md bg-blue-500 text-white font-fira px-4 py-1.5'>Tambah Anggota</button>
                      <button className='rounded-md bg-emerald-500 text-white font-fira px-4 py-1.5'>Cetak Anggota</button>
                                        ...
                                        <Table.Dropdown>
                                            <Menu>
                                                <Table.DropdownButton>
                                                    <Table.DropdownButton>Lihat</Table.DropdownButton>
                                                    <Table.DropdownButton onClick={() => editModalHandler(item)}>Edit</Table.DropdownButton>
