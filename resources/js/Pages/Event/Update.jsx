...
export default function Update({event}) {
  ...
  const { data, setData, patch, errors, reset, progress } = useForm({
    id:'', judul: '', tanggal_mulai: '', tanggal_berakhir: '', penyelenggara: '', thumbnail: '', kontent:''
  })
      ...
      <div className={clsx(
        ...
        <div className='border border-emerald-400 py-4 px-4 grid grid-cols-2 gap-3 overflow-y-auto min-h-[80%] items-start rounded-lg'>
          ...
          <div>
            ...
            <CKEDITOR style={'height:900px;'} handleChange={(e) => { setData({ ...data, kontent: e }) }} data={ event.kontent} />
          </div>
          </div>
            <div>
          <form action="" onSubmit={submitHanler} encType={'multipart/form-data'}>
            ...
            <input defaultValue={data.judul} ref={input} onChange={changeHandler} name='judul'  className=' my-1.5 w-full rounded-md border border-emerald-500 text-emerald-400 font-fira px-4 py-1.5 outline-none focus:ring focus:ring-emerald-400/30 ' type={'text'} placeholder='Judul Event' />
            ...
            <input defaultValue={data.tanggal_mulai} ref={input} onChange={changeHandler} name='tanggal_mulai'  className=' my-1.5 w-full rounded-md border border-emerald-500 text-emerald-400 font-fira px-4 py-1.5 outline-none focus:ring focus:ring-emerald-400/30 ' type={'date'}/>
            ...
            <input defaultValue={data.tanggal_berakhir} ref={input} onChange={changeHandler} name='tanggal_berakhir'  className=' my-1.5 w-full rounded-md border border-emerald-500 text-emerald-400 font-fira px-4 py-1.5 outline-none focus:ring focus:ring-emerald-400/30 ' type={'date'}/>
            ...
            <input defaultValue={data.penyelenggara} ref={input} onChange={changeHandler} name='penyelenggara'  className=' my-1.5 w-full rounded-md border border-emerald-500 text-emerald-400 font-fira px-4 py-1.5 outline-none focus:ring focus:ring-emerald-400/30 ' type={'text'} placeholder='Penyelenggara' />
            ...
            <input  ref={input} onChange={e => setData({...data, thumbnail:e.target.files[0]})} name='thumbnail'  className=' my-1.5 w-full rounded-md border border-emerald-500 text-emerald-400 font-fira px-4 py-1.5 outline-none focus:ring focus:ring-emerald-400/30 ' type={'file'} placeholder='Thumbnail' />
